...
const EditListing = () => {
    ...
    useEffect(() => {
      ...
      async function fetchListing(){
        ...
        if(docSnap.exists){
          ...
          setFormData({...docSnap.data()})  //saving the old values here
    ...
    return (
        <main className='mx-auto px-2 max-w-md pb-20'>
            ...
            <h1 className='text-lg text-center mt-1 font-bold text-[#457b9d]'>{`${formData.name} , ${formData.city}`}</h1>
            ...
            <form action="" onSubmit={onSubmit}>
                ...
                {!geoLocationEnabled && (
                    <div className='w-full flex justify-between items-center'>
                        <div className='w-[45%]'>
                            ...
                            <input type="number" id='latitude' value={latitude} onChange={onChange} step={"any"} min={-90} max={90} placeholder='Enter pin code' required className='w-full px-4 py-2 text-lg text-gray-700 bg-white border border-gray-300 rounded-lg transition duration-150 ease-in-out focus:text-gray-700 focus:bg-white focus:border-slate-600' />
                        </div>
                        <div className='w-[45%]'>
                            <p className='text-lg mt-6 mb-1' >Longitude (in °):</p>
                            <input type="number" id='longitude' value={longitude} step={"any"} onChange={onChange} min={-180} max={180} placeholder='Enter pin code' required className='w-full px-4 py-2 text-lg text-gray-700 bg-white border border-gray-300 rounded-lg transition duration-150 ease-in-out focus:text-gray-700 focus:bg-white focus:border-slate-600' />
                        </div>
                    </div>
                )}
                ...
                <button type='submit' className="w-full mt-6 uppercase flex justify-center items-center mx-auto text-white bg-blue-600 border-0 py-2 px-8 focus:outline-none hover:bg-blue-700 rounded text-base shadow-md focus:shadow-lg transition duration-150 ease-in-out">Update listing</button>
