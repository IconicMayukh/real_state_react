...
const Offers = () => {
  ...
  useEffect(() => {
    async function fetchListings(){
      try {
        ...
        const q = query(listingRef, where("discount","==",true), orderBy("timestamp", "desc"), limit(8))
  ...
  async function fetchMoreListings(){
    try {
      const listingRef = collection(db,"listings")
      const q = query(listingRef, where("discount","==",true), orderBy("timestamp", "desc"), startAfter(lastFetchListing), limit(4))
  ...
  return (
    <div className='max-w-[90%] mx-auto pt-4 space-y-6'>
      {listings.length === 0 && (
        <h2 className='px-3 text-xl mt-2 font-semibold'>Sorry there are no offers at this moment!</h2>
      )}
        {listings && listings.length > 0 && (
          <div className='mb-6'>
            <h2 className='px-3 text-2xl mt-2 mb-2 font-semibold text-center'>Offers</h2>
